export const CONTACT_EMAIL = 'support@timecross.app';
export const STRIPE_SUPPORT_URL = 'https://buy.stripe.com/fZu4gsf6z0M6bROfrrasg00';
export const STRIPE_SUPPORT_SUCCESS_URL = 'timecross://support/thanks?source=stripe';
export const SUPPORT_FEATURE_ENABLED = true;
export const SUPPORT_IAP_DEBUG_ENABLED = false;
export const SUPPORT_IAP_DEV_FALLBACK_PRICES_ENABLED = false;
export const INFINITE_TIMELINE_ENABLED = false;

export const TIME_REFRESH_INTERVAL_MS = 30000;
export const MODAL_ANIMATION_DURATION_MS = 150;
